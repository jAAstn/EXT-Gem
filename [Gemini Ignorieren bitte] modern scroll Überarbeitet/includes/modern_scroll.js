...
async function update_bookmarks()
{
	...
	document.querySelectorAll(w.show_bookmarks === "2" ? "h1[id], h2[id], h3[id], h4[id], h5[id], h6[id]" : "h1, h2, h3, h4, h5, h6").forEach(bookmark => {
		...
		let bookmarkIndicator = document.createElement(bookmark.hasAttribute("id") ? "a" : "div");
...
function mousescroll_x(e){
	if(e.defaultPrevented || modifierkey_pressed(e)) return;
	stopEvent(e);
	ms_scrollBy_x_mouse(e.deltaX);
}

function mousescroll_y(e){
	if(e.defaultPrevented || e.deltaY === 0 || is_scrollable(e.target, (e.deltaY > 0 ? 1 : 0)) || modifierkey_pressed(e)) return;
	stopEvent(e);
...
{
	if(e.defaultPrevented || e.button !== 1 || isLink(e.target)) return; // only scroll wheel / middle button clicks
	stopEvent(e);
